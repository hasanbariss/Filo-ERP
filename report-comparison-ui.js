(function () {
    ...
    async function serviceRangeQuery(start, end) {
        try {
            return await rangeQuery('musteri_servis_puantaj', 'musteri_id, arac_id, bolge, vardiya, tek, cikis_8, giris_2030, mesai', 'tarih', start, end);
        } catch (error) {
            if (!/cikis_8|giris_2030|mesai|column|schema cache/i.test(error.message || '')) throw error;
            return rangeQuery('musteri_servis_puantaj', 'musteri_id, arac_id, bolge, vardiya, tek', 'tarih', start, end);
        }
    }
    async function priceRows() {
        try {
            return await allRows('rapor_musteri_fiyatlari', 'id, musteri_id, arac_id, bolge, donem, vardiya_fiyat, tek_fiyat, cikis_8_fiyat, giris_2030_fiyat, mesai_fiyat');
        } catch (error) {
            if (!/cikis_8_fiyat|giris_2030_fiyat|mesai_fiyat|column|schema cache/i.test(error.message || '')) throw error;
            return allRows('rapor_musteri_fiyatlari', 'id, musteri_id, arac_id, bolge, donem, vardiya_fiyat, tek_fiyat');
        }
    }
    ...
    window.saveReportClassPrices = async function() {
        ...
        try {
            ...
            for(var item of plan){
                var response;
                if(item.id){response=await window.supabaseClient.from('rapor_musteri_fiyatlari').update(item.payload).eq('id',item.id).select('id');}
                else{response=await window.supabaseClient.from('rapor_musteri_fiyatlari').insert([item.payload]).select('id');}
                if(response.error)throw response.error;
    ...
    window.saveReportServicePrices = async function (customerIndex, detailIndex, button) {
        ...
        try {
            var response;
            if (change.id) response = await window.supabaseClient.from('rapor_musteri_fiyatlari').update(change.payload).eq('id', change.id);
            else response = await window.supabaseClient.from('rapor_musteri_fiyatlari').insert([change.payload]);
            if (response.error) throw response.error;
