(function () {
    ...
    async function serviceRows(start, end) {
        var result = await window.supabaseClient.from('musteri_servis_puantaj').select('id, tarih, arac_id, sofor_id, vardiya, tek, cikis_8, giris_2030, mesai').gte('tarih', start).lte('tarih', end);
        if (result.error && /sofor_id|column|schema cache/i.test(result.error.message || '')) {
            state.snapshotAvailable = false;
            result = await window.supabaseClient.from('musteri_servis_puantaj').select('id, tarih, arac_id, vardiya, tek, cikis_8, giris_2030, mesai').gte('tarih', start).lte('tarih', end);
        }
        if (result.error) throw result.error;
        return result.data || [];
    }
    function prepareMarkup() {
        ...
        if (puantaj && puantaj.dataset.operationalReady !== 'true') {
            ...
            puantaj.innerHTML = '<div class="personnel-operation-head"><div><span>OPERASYONEL PUANTAJ</span><h3>Seferden Gelen Günlük Çalışma</h3><p>Araç seferleri şoför bazında görünür; bordroya yalnızca onaylı kayıtlar etki eder.</p></div><span class="personnel-safe-badge"><i data-lucide="shield-check"></i>Bordroya otomatik yazılmaz</span></div><div class="personnel-operation-summary" id="personnel-operation-summary"></div><div class="personnel-operation-toolbar"><label><i data-lucide="search"></i><input id="personnel-operation-search" type="search" placeholder="Şoför veya plaka ara" oninput="window.applyPersonnelOperationFilters()"></label><select id="personnel-operation-class" onchange="window.applyPersonnelOperationFilters()"><option value="">Tüm araç sınıfları</option><option value="TAKSİ">Taksi</option><option value="16+1">16+1 Minibüs</option><option value="27+1">27+1 Midibüs</option><option value="46+1">46+1 Otobüs</option><option value="SINIFLANDIRILMAMIŞ">Sınıflandırılmamış</option></select><select id="personnel-operation-sort" onchange="window.applyPersonnelOperationFilters()"><option value="date-desc">Tarih · yeniden eskiye</option><option value="operation-desc">Sefer · yüksekten</option><option value="driver-asc">Şoför · A–Z</option></select></div><div id="personnel-operation-note" class="personnel-operation-note"></div><div class="personnel-operation-table-wrap"><table class="personnel-operation-table"><thead><tr><th>Tarih</th><th>Şoför</th><th>Araç / Sınıf</th><th>Vardiya</th><th>Tek</th><th>Diğer</th><th>Toplam</th><th>Kaynak</th></tr></thead><tbody id="personnel-operation-tbody"></tbody></table></div>';
        }
    ...
    function aggregate(current, previous) {
        ...
        current.forEach(function (row) {
            ...
            var group = groups.get(key) || { driverId:key, driver:row.driver, plates:new Set(), classes:new Set(), days:new Set(), shifts:0, trips:0, operations:0, source:row.source };
    ...
    function renderDaily() {
        ...
        var rows=state.dailyRows.filter(function(row){return (!query||(row.driver+' '+row.plate).toLocaleLowerCase('tr-TR').includes(query))&&(!vehicleClass||row.vehicleClass===vehicleClass);});
    ...
    window.fetchPersonnelOperations=async function(){
        ...
        try{
            var results=await Promise.all([window.supabaseClient.from('soforler').select('id, ad_soyad'),vehicles(),serviceRows(bounds.start,bounds.end),serviceRows(bounds.previousStart,bounds.previousEnd)]);
            ...
            var currentRows=buildDaily(results[2],vehicleMap,driverMap);
            var previousRows=buildDaily(results[3],vehicleMap,driverMap);
            ...
            if(window.lucide)window.lucide.createIcons();
        }catch(error){console.error('[Personel Operasyon]',error);var tbody=document.getElementById('personnel-operation-tbody');if(tbody)tbody.innerHTML='<tr><td colspan="8"><div class="empty-state">Operasyon puantajı yüklenemedi.</div></td></tr>';}
    };
    ...
    window.fetchPersonnelPayrollOverview=async function(){
        ...
        try{
            ...
            var finance=new Map();(res[2].data||[]).forEach(function(row){var item=finance.get(String(row.sofor_id))||{avans:0,ceza:0,haciz:0};var type=String(row.islem_turu||'').toLocaleLowerCase('tr-TR');if(type.includes('avans'))item.avans+=number(row.tutar);else if(type.includes('ceza'))item.ceza+=number(row.tutar);else if(type.includes('haciz'))item.haciz+=number(row.tutar);finance.set(String(row.sofor_id),item);});
        ...
        }catch(error){console.error('[Personel Bordro]',error);}
    };
